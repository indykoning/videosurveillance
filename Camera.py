...
class P2PCam():
    def __init__(self, host_ip, target_ip):
        ...
        self.jpeg = None
        self.MESSAGE_43 = bytearray([0x00,0x00,0xb0,0x02,0x82,0x00,0x00,0x27,0x00,0x01,0x00,0x00,0x00,0x4d,0x61,0x63,0x49,0x50,0x3d,0x42,0x43,0x2d,0x41,0x45,0x2d,0x43,0x35,0x2d,0x37,0x43,0x2d,0x37,0x37,0x2d,0x37,0x42,0x2b,0x31,0x36,0x34,0x36,0x37,0x3b])
        ...
        self.MESSAGE_CONTINUE_BEGIN = bytearray([0x00,0x00,0xff,0x02,0x12,0x00,0x00,0xff,0x00,0x01,0x00,0x00,0x00,0xa0,0xaa,0xa4,0xad,0xd4,0xd8,0xd2,0xba,0xac,0xb8,0xd4])
        ...
        self.CONTINUE_LIST_1 = bytearray([0xd8,0xdf, 0xdb,0xde, 0xda,0xd1, 0xdd,0xd0, 0xdc,0xd9, 0xdf, 0xd8, 0xde,0xdb, 0xd1,0xda, 0xd0, 0xdd, 0xd9,0xdc])
        self.sock = None
    ...
    def loop(self):
        try:
            ...
            while socket_error == False:
                ...
                fragmentIndex += 1
                if (fragments_received <= self.NB_FRAGMENTS_TO_ACCUMULATE):
                    ...
                    if nbbytes >= 17:
                        ...
                        # additional data fragment : just drop the 4 bytes header and concatenate to already received data
                            ...
                            if ((self.byteToInt(chunk[0]) == lastFragmentId+1) or (self.byteToInt(chunk[0]) == 0) and (lastFragmentId==255)):
                                msg += chunk[4:]
                    ...
                    if SOI_index!=-1 and EOI_index!=-1:
                        ...
                        self.jpeg = msg[SOI_index:EOI_index+2]
                        msg = msg[EOI_index+2:]
                        try:
            ...
            #            print("no image found in stream among %d bytes"% len(msg))
                ...
                if (fragmentIndex%5) == 0:
                    ...
                    if (nbDigits == 1):
                        ...
                        tmp.append(self.CONTINUE_LIST_1[base_index+continue_index[0]])
                        continue_index[0] += 1
                        ...
                        tmp.append(self.CONTINUE_LIST_2[continue_index[1]])
                        tmp.append(self.CONTINUE_LIST_1[base_index+continue_index[0]])
                        continue_index[0] += 1
                        if continue_index[0] == 2:
                            continue_index[1] += 1
                            continue_index[0] = 0
                        if continue_index[1] == len(self.CONTINUE_LIST_2):
                            nbDigits += 1
                        ...
                        tmp.append(self.CONTINUE_LIST_2[continue_index[2]])
                        tmp.append(self.CONTINUE_LIST_2[continue_index[1]])
                        tmp.append(self.CONTINUE_LIST_1[base_index+continue_index[0]])
                        # update digit 0
                        continue_index[0] += 1
                        ...
                        if continue_index[2] == len(self.CONTINUE_LIST_2):
                            nbDigits += 1
                        ...
                        tmp.append(self.CONTINUE_LIST_2[continue_index[3]])
                        tmp.append(self.CONTINUE_LIST_2[continue_index[2]])
                        tmp.append(self.CONTINUE_LIST_2[continue_index[1]])
                        tmp.append(self.CONTINUE_LIST_1[base_index+continue_index[0]])
                        # update digit 0
                        ...
                        if continue_index[2] == len(self.CONTINUE_LIST_2):
                            ...
                            continue_index[0] = 0 # start at d9
                        # check for adding one more digit
                        if continue_index[3] == len(self.CONTINUE_LIST_2):
                            nbDigits += 1
                        ...
                        tmp.append(self.CONTINUE_LIST_2[continue_index[4]])
                        tmp.append(self.CONTINUE_LIST_2[continue_index[3]])
                        tmp.append(self.CONTINUE_LIST_2[continue_index[2]])
                        tmp.append(self.CONTINUE_LIST_2[continue_index[1]])
                        tmp.append(self.CONTINUE_LIST_1[base_index+continue_index[0]])
                        ...
                        if continue_index[3] == len(self.CONTINUE_LIST_2):
                            ...
                            continue_index[1] = 0 # start at d9
                            continue_index[0] = 0 # start at d9
                        if continue_index[4] == len(self.CONTINUE_LIST_2):
                            # restart sequence
